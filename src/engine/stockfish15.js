  ...
  var Stockfish = (function () {
    ...
    return (
      function (Stockfish) {
        ...
        if (q) {
          A = n ? require("path").dirname(A) + "/" : __dirname + "/"; ma = function (a, b) { oa || (oa = require("fs")); pa || (pa = require("path")); a = pa.normalize(a); return oa.readFileSync(a, b ? null : "utf8") }; na = function (a) { a = ma(a, !0); a.buffer || (a = new Uint8Array(a)); assert(a.buffer); return a }; 1 < process.argv.length && (ia = process.argv[1].replace(/\\/g, "/")); ha = process.argv.slice(2); process.on("uncaughtException", function (a) { if (!(a instanceof qa)) throw a; }); process.on("unhandledRejection", B); ja = function (a, b) {
            if (ra()) throw process.exitCode =
              a, b; process.exit(a)
          }; d.inspect = function () { return "[Emscripten Module object]" }; var sa; try { sa = require("worker_threads") } catch (a) { throw console.error('The "worker_threads" module is not supported in this node.js build - perhaps a newer version is needed?'), a; } global.Worker = sa.Worker
        } else if (ka || n) n ? A = self.location.href : "undefined" !== typeof document && document.currentScript && (A = document.currentScript.src), _scriptDir && (A = _scriptDir), 0 !== A.indexOf("blob:") ? A = A.substr(0, A.lastIndexOf("/") + 1) : A = "", q ? (ma = function (a,
        ...
        var l = {
          za: [], ya: [], Ta: [], Db: function () { }, gb: function () { for (var a = T(228), b = 0; 57 > b; ++b)N[a / 4 + b] = 0; M[a + 12 >> 2] = a; b = a + 152; M[b >> 2] = b; var c = T(512); for (b = 0; 128 > b; ++b)N[c / 4 + b] = 0; Atomics.store(N, a + 100 >> 2, c); Atomics.store(N, a + 40 >> 2, a); cb(a, !n, 1); db(a) }, hb: function () { l.receiveObjectTransfer = l.kb; l.threadInit = l.pb; l.threadCancel = l.ob; l.threadExit = l.$a; l.setExitStatus = l.mb }, wa: {}, Sa: [], Ya: function () { for (; 0 < l.Sa.length;)l.Sa.pop()(); eb() }, Za: function (a, b) {
            Atomics.store(N, a + 56 >> 2, 1); Atomics.store(N, a + 60 >> 2, 0); l.Ya();
            Atomics.store(N, a + 4 >> 2, b); Atomics.store(N, a + 0 >> 2, 1); $a(a + 0, 2147483647); cb(0, 0, 0)
          }, mb: function (a) { za = a }, $a: function (a) { var b = fb(); b && (l.Za(b, a), z && postMessage({ cmd: "exit" })) }, ob: function () { l.Za(fb(), -1); postMessage({ cmd: "cancelDone" }) }, Ra: function () { for (var a in l.wa) { var b = l.wa[a]; b && b.worker && l.Ka(b.worker) } l.wa = {}; for (a = 0; a < l.za.length; ++a) { var c = l.za[a]; c.terminate() } l.za = []; for (a = 0; a < l.ya.length; ++a)c = l.ya[a], b = c.va, l.Pa(b), c.terminate(); l.ya = [] }, Pa: function (a) {
            if (a) {
              if (a.xa) {
                var b = M[a.xa + 100 >>
                  2]; M[a.xa + 100 >> 2] = 0; gb(b); gb(a.xa)
              } a.xa = 0; a.Oa && a.Aa && gb(a.Aa); a.Aa = 0; a.worker && (a.worker.va = null)
            }
          }, Ka: function (a) { l.lb(function () { delete l.wa[a.va.xa]; l.za.push(a); l.ya.splice(l.ya.indexOf(a), 1); l.Pa(a.va); a.va = void 0 }) }, lb: function (a) { M[hb >> 2] = 0; try { a() } finally { M[hb >> 2] = 1 } }, kb: function () { }, pb: function () { for (var a in l.Ta) l.Ta[a]() }, ib: function (a, b) {
            a.onmessage = function (c) {
              var e = c.data, h = e.cmd; a.va && (l.bb = a.va.xa); if (e.targetThread && e.targetThread != fb()) {
                var g = l.wa[e.Ib]; g ? g.worker.postMessage(c.data,
                  e.transferList) : E('Internal error! Worker sent a message "' + h + '" to target pthread ' + e.targetThread + ", but that thread no longer exists!")
              } else if ("processQueuedMainThreadWork" === h) ib(); else if ("spawnThread" === h) jb(c.data); else if ("cleanupThread" === h) bb(e.thread); else if ("killThread" === h) {
                c = e.thread; if (z) throw "Internal Error! killThread() can only ever be called from main application thread!"; if (!c) throw "Internal Error! Null pthread_ptr in killThread!"; M[c + 12 >> 2] = 0; e = l.wa[c]; delete l.wa[c]; e.worker.terminate();
                l.Pa(e); l.ya.splice(l.ya.indexOf(e.worker), 1); e.worker.va = void 0
              } else if ("cancelThread" === h) { c = e.thread; if (z) throw "Internal Error! cancelThread() can only ever be called from main application thread!"; if (!c) throw "Internal Error! Null pthread_ptr in cancelThread!"; l.wa[c].worker.postMessage({ cmd: "cancel" }) } else if ("loaded" === h) a.loaded = !0, b && b(a), a.Ea && (a.Ea(), delete a.Ea); else if ("print" === h) ta("Thread " + e.threadId + ": " + e.text); else if ("printErr" === h) E("Thread " + e.threadId + ": " + e.text); else if ("alert" ===
                h) alert("Thread " + e.threadId + ": " + e.text); else if ("exit" === h) a.va && Atomics.load(N, a.va.xa + 64 >> 2) && l.Ka(a); else if ("exitProcess" === h) try { kb(e.returnCode) } catch (k) { if (k instanceof qa) return; throw k; } else "cancelDone" === h ? l.Ka(a) : "objectTransfer" !== h && ("setimmediate" === c.data.target ? a.postMessage(c.data) : E("worker sent an unknown command " + h)); l.bb = void 0
            }; a.onerror = function (c) { E("pthread sent an error! " + c.filename + ":" + c.lineno + ": " + c.message) }; q && (a.on("message", function (c) { a.onmessage({ data: c }) }),
              a.on("error", function (c) { a.onerror(c) }), a.on("exit", function () { })); a.postMessage({ cmd: "load", urlOrBlob: d.mainScriptUrlOrBlob || _scriptDir, wasmMemory: F, wasmModule: wa })
          }, ab: function () { var a = la("stockfish.worker.js"); l.za.push(new Worker(a)) }, eb: function () { 0 == l.za.length && (l.ab(), l.ib(l.za[0])); return l.za.pop() }, wb: function (a) { for (a = performance.now() + a; performance.now() < a;); }
        }; d.establishStackSpace = function (a, b) { lb(a, b); mb(a) }; d.invokeEntryPoint = function (a, b) { return nb.apply(null, [a, b]) }; var ob;
        ...
        function qb(a, b, c) {
          if (0 >= a || a > Ha.length || a & 1) return -28; if (ka) { if (Atomics.load(M, a >> 2) != b) return -6; var e = performance.now(); c = e + c; for (Atomics.exchange(M, ab >> 2, a); ;) { e = performance.now(); if (e > c) return Atomics.exchange(M, ab >> 2, 0), -73; e = Atomics.exchange(M, ab >> 2, 0); if (0 == e) break; ib(); if (Atomics.load(M, a >> 2) != b) return -6; Atomics.exchange(M, ab >> 2, a) } return 0 } a = Atomics.wait(M, a >> 2, b, c); if ("timed-out" === a) return -73; if ("not-equal" === a) return -6; if ("ok" === a) return 0; throw "Atomics.wait returned an unexpected value " +
            a;
        } function rb() { q || n || (ua || (ua = {}), ua["Blocking on the main thread is very dangerous, see https://emscripten.org/docs/porting/pthreads.html#blocking-on-the-main-browser-thread"] || (ua["Blocking on the main thread is very dangerous, see https://emscripten.org/docs/porting/pthreads.html#blocking-on-the-main-browser-thread"] = 1, E("Blocking on the main thread is very dangerous, see https://emscripten.org/docs/porting/pthreads.html#blocking-on-the-main-browser-thread"))) }
        function sb(a, b) {
          if (!a) return E("pthread_join attempted on a null thread pointer!"), 71; if (z && fb() == a) return E("PThread " + a + " is attempting to join to itself!"), 16; if (!z && tb() == a) return E("Main thread " + a + " is attempting to join to itself!"), 16; if (M[a + 12 >> 2] !== a) return E("pthread_join attempted on thread " + a + ", which does not point to a valid thread, or does not exist anymore!"), 71; if (Atomics.load(N, a + 64 >> 2)) return E("Attempted to join thread " + a + ", which was already detached!"), 28; for (rb(); ;) {
            var c =
              Atomics.load(N, a >> 2); if (1 == c) return c = Atomics.load(N, a + 4 >> 2), b && (M[b >> 2] = c), Atomics.store(N, a + 64 >> 2, 1), z ? postMessage({ cmd: "cleanupThread", thread: a }) : bb(a), 0; ub(); z || ib(); qb(a, c, z ? 100 : 1)
          }
        } var vb = [null, [], []], wb = {}; function xb(a, b, c) { return z ? U(2, 1, a, b, c) : 0 } function yb(a, b, c) { return z ? U(3, 1, a, b, c) : 0 } function zb(a, b, c) { if (z) return U(4, 1, a, b, c) } function Ab() { if (z) return U(5, 1); B() } var Bb = []; function Cb(a) { if (z) return U(6, 1, a); noExitRuntime = !1; R = 0; kb(a) }
        ...
        function Pb(a, b, c, e, h) {
          function g(C) { var D = 0, w = 0; C && (w = p.response ? p.response.byteLength : 0, D = T(w), I.set(new Uint8Array(p.response), D)); N[a + 12 >> 2] = D; X(a + 16, w) } var k = N[a + 8 >> 2]; if (k) {
            var u = H(k), t = a + 112, v = H(t); v || (v = "GET"); var y = N[t + 52 >> 2], O = N[t + 56 >> 2], J = !!N[t + 60 >> 2], f = N[t + 68 >> 2], r = N[t + 72 >> 2]; k = N[t + 76 >> 2]; var x = N[t + 80 >> 2], G = N[t + 84 >> 2]; t = N[t + 88 >> 2]; var P = !!(y & 1), V = !!(y & 2); y = !!(y & 64); f = f ? H(f) : void 0; r = r ? H(r) : void 0; var Ea = x ? H(x) : void 0, p = new XMLHttpRequest; p.withCredentials = J; p.open(v, u, !y, f, r); y || (p.timeout =
              O); p.vb = u; p.responseType = "arraybuffer"; x && p.overrideMimeType(Ea); if (k) for (; ;) { v = N[k >> 2]; if (!v) break; u = N[k + 4 >> 2]; if (!u) break; k += 8; v = H(v); u = H(u); p.setRequestHeader(v, u) } Mb.push(p); N[a + 0 >> 2] = Mb.length; k = G && t ? I.slice(G, G + t) : null; p.onload = function (C) { g(P && !V); var D = p.response ? p.response.byteLength : 0; X(a + 24, 0); D && X(a + 32, D); L[a + 40 >> 1] = p.readyState; L[a + 42 >> 1] = p.status; p.statusText && K(p.statusText, I, a + 44, 64); 200 <= p.status && 300 > p.status ? b && b(a, p, C) : c && c(a, p, C) }; p.onerror = function (C) {
                g(P); var D = p.status; X(a +
                  24, 0); X(a + 32, p.response ? p.response.byteLength : 0); L[a + 40 >> 1] = p.readyState; L[a + 42 >> 1] = D; c && c(a, p, C)
                ...
                function (C) { L[a + 40 >> 1] = p.readyState; 2 <= p.readyState && (L[a + 42 >> 1] = p.status); h && h(a, p, C) }; try { p.send(k) } catch (C) { c && c(a, p, C) }
        ...
        function Rb(a, b, c) {
          var e = Ob; if (e) {
            var h = N[a + 112 + 64 >> 2]; h || (h = N[a + 8 >> 2]); h = H(h); try {
              var g = e.transaction(["FILES"], "readonly").objectStore("FILES").get(h); g.onsuccess = function (k) { if (k.target.result) { k = k.target.result; var u = k.byteLength || k.length, t = T(u); I.set(new Uint8Array(k), t); N[a + 12 >> 2] = t; X(a + 16, u); X(a + 24, 0); X(a + 32, u); L[a + 40 >> 1] = 4; L[a + 42 >> 1] = 200; K("OK", I, a + 44, 64); b(a, 0, k) } else L[a + 40 >> 1] = 4, L[a + 42 >> 1] = 404, K("Not Found", I, a + 44, 64), c(a, 0, "no data") }; g.onerror = function (k) {
                L[a + 40 >> 1] = 4; L[a + 42 >> 1] = 404;
                K("Not Found", I, a + 44, 64); c(a, 0, k)
              }
            } catch (k) { c(a, 0, k) }
        ...
        function oc(a, b, c, e) {
          function h(f, r, x) { for (f = "number" === typeof f ? f.toString() : f || ""; f.length < r;)f = x[0] + f; return f } function g(f, r) { return h(f, r, "0") } function k(f, r) { function x(P) { return 0 > P ? -1 : 0 < P ? 1 : 0 } var G; 0 === (G = x(f.getFullYear() - r.getFullYear())) && 0 === (G = x(f.getMonth() - r.getMonth())) && (G = x(f.getDate() - r.getDate())); return G } function u(f) {
            switch (f.getDay()) {
              case 0: return new Date(f.getFullYear() - 1, 11, 29); case 1: return f; case 2: return new Date(f.getFullYear(), 0, 3); case 3: return new Date(f.getFullYear(),
                0, 2); case 4: return new Date(f.getFullYear(), 0, 1); case 5: return new Date(f.getFullYear() - 1, 11, 31); case 6: return new Date(f.getFullYear() - 1, 11, 30)
            }
          } function t(f) { f = nc(new Date(f.U + 1900, 0, 1), f.Na); var r = new Date(f.getFullYear() + 1, 0, 4), x = u(new Date(f.getFullYear(), 0, 4)); r = u(r); return 0 >= k(x, f) ? 0 >= k(r, f) ? f.getFullYear() + 1 : f.getFullYear() : f.getFullYear() - 1 } var v = M[e + 40 >> 2]; e = {
            sb: M[e >> 2], rb: M[e + 4 >> 2], La: M[e + 8 >> 2], Fa: M[e + 12 >> 2], Ca: M[e + 16 >> 2], U: M[e + 20 >> 2], Ma: M[e + 24 >> 2], Na: M[e + 28 >> 2], Jb: M[e + 32 >> 2], qb: M[e + 36 >>
              2], tb: v ? H(v) : ""
          }; c = H(c); v = { "%c": "%a %b %d %H:%M:%S %Y", "%D": "%m/%d/%y", "%F": "%Y-%m-%d", "%h": "%b", "%r": "%I:%M:%S %p", "%R": "%H:%M", "%T": "%H:%M:%S", "%x": "%m/%d/%y", "%X": "%H:%M:%S", "%Ec": "%c", "%EC": "%C", "%Ex": "%m/%d/%y", "%EX": "%H:%M:%S", "%Ey": "%y", "%EY": "%Y", "%Od": "%d", "%Oe": "%e", "%OH": "%H", "%OI": "%I", "%Om": "%m", "%OM": "%M", "%OS": "%S", "%Ou": "%u", "%OU": "%U", "%OV": "%V", "%Ow": "%w", "%OW": "%W", "%Oy": "%y" }; for (var y in v) c = c.replace(new RegExp(y, "g"), v[y]); var O = "Sunday Monday Tuesday Wednesday Thursday Friday Saturday".split(" "),
            J = "January February March April May June July August September October November December".split(" "); v = {
              "%a": function (f) { return O[f.Ma].substring(0, 3) }, "%A": function (f) { return O[f.Ma] }, "%b": function (f) { return J[f.Ca].substring(0, 3) }, "%B": function (f) { return J[f.Ca] }, "%C": function (f) { return g((f.U + 1900) / 100 | 0, 2) }, "%d": function (f) { return g(f.Fa, 2) }, "%e": function (f) { return h(f.Fa, 2, " ") }, "%g": function (f) { return t(f).toString().substring(2) }, "%G": function (f) { return t(f) }, "%H": function (f) {
                return g(f.La,
                  2)
              }, "%I": function (f) { f = f.La; 0 == f ? f = 12 : 12 < f && (f -= 12); return g(f, 2) }, "%j": function (f) { return g(f.Fa + kc(jc(f.U + 1900) ? lc : mc, f.Ca - 1), 3) }, "%m": function (f) { return g(f.Ca + 1, 2) }, "%M": function (f) { return g(f.rb, 2) }, "%n": function () { return "\n" }, "%p": function (f) { return 0 <= f.La && 12 > f.La ? "AM" : "PM" }, "%S": function (f) { return g(f.sb, 2) }, "%t": function () { return "\t" }, "%u": function (f) { return f.Ma || 7 }, "%U": function (f) {
                var r = new Date(f.U + 1900, 0, 1), x = 0 === r.getDay() ? r : nc(r, 7 - r.getDay()); f = new Date(f.U + 1900, f.Ca, f.Fa); return 0 >
                  k(x, f) ? g(Math.ceil((31 - x.getDate() + (kc(jc(f.getFullYear()) ? lc : mc, f.getMonth() - 1) - 31) + f.getDate()) / 7), 2) : 0 === k(x, r) ? "01" : "00"
              }, "%V": function (f) { var r = new Date(f.U + 1901, 0, 4), x = u(new Date(f.U + 1900, 0, 4)); r = u(r); var G = nc(new Date(f.U + 1900, 0, 1), f.Na); return 0 > k(G, x) ? "53" : 0 >= k(r, G) ? "01" : g(Math.ceil((x.getFullYear() < f.U + 1900 ? f.Na + 32 - x.getDate() : f.Na + 1 - x.getDate()) / 7), 2) }, "%w": function (f) { return f.Ma }, "%W": function (f) {
                var r = new Date(f.U, 0, 1), x = 1 === r.getDay() ? r : nc(r, 0 === r.getDay() ? 1 : 7 - r.getDay() + 1); f = new Date(f.U +
                  1900, f.Ca, f.Fa); return 0 > k(x, f) ? g(Math.ceil((31 - x.getDate() + (kc(jc(f.getFullYear()) ? lc : mc, f.getMonth() - 1) - 31) + f.getDate()) / 7), 2) : 0 === k(x, r) ? "01" : "00"
              }, "%y": function (f) { return (f.U + 1900).toString().substring(2) }, "%Y": function (f) { return f.U + 1900 }, "%z": function (f) { f = f.qb; var r = 0 <= f; f = Math.abs(f) / 60; return (r ? "+" : "-") + String("0000" + (f / 60 * 100 + f % 60)).slice(-4) }, "%Z": function (f) { return f.tb }, "%%": function () { return "%" }
            }; for (y in v) c.includes(y) && (c = c.replace(new RegExp(y, "g"), v[y](e))); y = pc(c); if (y.length >
              b) return 0; Ha.set(y, a); return y.length - 1
        } function qc(a) { try { a() } catch (b) { B(b) } } var Y = 0, Z = null, rc = 0, sc = [], tc = {}, uc = {}, vc = 0, wc = null, xc = [], yc = []; function zc(a) { var b = {}, c; for (c in a) (function (e) { var h = a[e]; b[e] = "function" === typeof h ? function () { sc.push(e); try { return h.apply(null, arguments) } finally { if (!ya) { var g = sc.pop(); assert(g === e); Z && 1 === Y && 0 === sc.length && (R += 1, Y = 0, qc(d._asyncify_stop_unwind), "undefined" !== typeof Fibers && Fibers.Kb(), wc && (wc(), wc = null)) } } } : h })(c); return b }
        ...
        var Jc = {
          c: function (a, b, c, e) { B("Assertion failed: " + H(a) + ", at: " + [b ? H(b) : "unknown filename", c, e ? H(e) : "unknown function"]) }, E: function (a, b) { Fc(a, b) }, o: function (a, b) { l.Sa.push(function () { dynCall_vi.apply(null, [a, b]) }) }, L: function (a, b, c, e) {
            if ("undefined" === typeof SharedArrayBuffer) return E("Current environment does not support SharedArrayBuffer, pthreads are not available!"), 6; if (!a) return E("pthread_create called with a null thread pointer!"), 28; var h = []; if (z && 0 === h.length) return Gc(687865856, a, b, c,
              e); var g = 0, k = 0; if (b && -1 != b) { var u = M[b >> 2]; u += 81920; g = M[b + 8 >> 2]; k = 0 !== M[b + 12 >> 2] } else u = 2097152; (b = 0 == g) ? g = Hc(16, u) : (g -= u, assert(0 < g)); for (var t = T(228), v = 0; 57 > v; ++v)N[(t >> 2) + v] = 0; M[a >> 2] = t; M[t + 12 >> 2] = t; a = t + 152; M[a >> 2] = a; c = { Aa: g, Ba: u, Oa: b, detached: k, nb: c, Ja: t, Da: e, ub: h }; return z ? (c.xb = "spawnThread", postMessage(c, h), 0) : jb(c)
          }, J: function (a) { z ? l.$a(a) : (l.Ya(), kb(a)); throw "unwind"; }, K: function (a, b) { return sb(a, b) }, h: xb, x: yb, y: zb, P: function (a) { delete Mb[a - 1] }, O: function (a, b) {
            if (a == b) postMessage({ cmd: "processQueuedMainThreadWork" });
            else if (z) postMessage({ targetThread: a, cmd: "processThreadQueue" }); else { a = (a = l.wa[a]) && a.worker; if (!a) return; a.postMessage({ cmd: "processThreadQueue" }) } return 1
          }, b: Ab, D: function (a, b) { if (0 === a) a = Date.now(); else if (1 === a || 4 === a) a = ob(); else return M[Ic() >> 2] = 28, -1; M[b >> 2] = a / 1E3 | 0; M[b + 4 >> 2] = a % 1E3 * 1E6 | 0; return 0 }, m: function (a, b, c) { Bb.length = 0; var e; for (c >>= 2; e = I[b++];)(e = 105 > e) && c & 1 && c++, Bb.push(e ? Ja[c++ >> 1] : M[c]), ++c; return Ya[a].apply(null, Bb) }, C: rb, l: function () { }, p: Cb, d: qb, e: $a, f: ob, s: function (a, b, c) {
            I.copyWithin(a,
              b, b + c)
          }, G: function (a, b, c) { Fb.length = b; c >>= 3; for (var e = 0; e < b; e++)Fb[e] = Ja[c + e]; return (0 > a ? Ya[-a - 1] : Ec[a]).apply(null, Fb) }, t: function () { B("OOM") }, H: function (a, b, c) { return Hb(a) ? Ib(a, b, c) : Kb(a, b, c) }, k: function () { }, F: function () { }, N: function (a, b, c) { R += 1; return setTimeout(function () { --R; W(function () { dynCall_vi.apply(null, [a, c]) }) }, b) }, n: function (a, b, c, e, h) {
            function g(w) { Pb(w, k, v, y, t) } function k(w, Mc) {
              Qb(w, Mc.response, function (xa) { --R; W(function () { r ? dynCall_vi.apply(null, [r, xa]) : b && b(xa) }, D) }, function (xa) {
                --R;
                W(function () { r ? dynCall_vi.apply(null, [r, xa]) : b && b(xa) }, D)
              })
            } function u(w) { Pb(w, O, v, y, t) } function t(w) { W(function () { P ? dynCall_vi.apply(null, [P, w]) : h && h(w) }, D) } function v(w) { --R; W(function () { x ? dynCall_vi.apply(null, [x, w]) : c && c(w) }, D) } function y(w) { W(function () { G ? dynCall_vi.apply(null, [G, w]) : e && e(w) }, D) } function O(w) { --R; W(function () { r ? dynCall_vi.apply(null, [r, w]) : b && b(w) }, D) } R += 1; var J = a + 112, f = H(J), r = N[J + 36 >> 2], x = N[J + 40 >> 2], G = N[J + 44 >> 2], P = N[J + 48 >> 2], V = N[J + 52 >> 2], Ea = !!(V & 4), p = !!(V & 32), C = !!(V & 16), D = !!(V &
              64); if ("EM_IDB_STORE" === f) f = N[J + 84 >> 2], Qb(a, I.slice(f, f + N[J + 88 >> 2]), O, v); else if ("EM_IDB_DELETE" === f) Sb(a, O, v); else if (C) { if (p) return 0; Pb(a, Ea ? k : O, v, y, t) } else Rb(a, O, p ? v : Ea ? g : u); return a
          }, r: function () { return Dc(async () => { var a = await d.queue.get(); const b = Da(a) + 1, c = T(b); K(a, I, c, b); return c }) }, I: function (a, b) {
            b >>= 2; b = {
              alpha: !!M[b], depth: !!M[b + 1], stencil: !!M[b + 2], antialias: !!M[b + 3], premultipliedAlpha: !!M[b + 4], preserveDrawingBuffer: !!M[b + 5], powerPreference: $b[M[b + 6]], failIfMajorPerformanceCaveat: !!M[b +
                7], jb: M[b + 8], Eb: M[b + 9], Ua: M[b + 10], cb: M[b + 11], Gb: M[b + 12], Hb: M[b + 13]
            }; a = Hb(a); return !a || b.cb ? 0 : Wb(a, b)
          }, v: dc, w: ec, g: function (a) { kb(a) }, i: fc, A: gc, q: hc, z: ic, M: function () { l.gb() }, a: F || d.wasmMemory, B: function () { d.pauseQueue() }, u: function (a, b, c, e) { return oc(a, b, c, e) }, j: function () { d.unpauseQueue() }
        };
        (function () {
          function a(g, k) { g = g.exports; g = zc(g); d.asm = g; Ma.unshift(d.asm.Q); l.Ta.push(d.asm.T); wa = k; z || Ua() } function b(g) { a(g.instance, g.module) } function c(g) { return Xa().then(function (k) { return WebAssembly.instantiate(k, e) }).then(g, function (k) { E("failed to asynchronously prepare wasm: " + k); B(k) }) } var e = { a: Jc }; z || Ta(); if (d.instantiateWasm) try { var h = d.instantiateWasm(e, a); return h = zc(h) } catch (g) { return E("Module.instantiateWasm callback failed with error: " + g), !1 } (function () {
            return va || "function" !==
              typeof WebAssembly.instantiateStreaming || Va() || "function" !== typeof fetch ? c(b) : fetch(S, { credentials: "same-origin" }).then(function (g) { return WebAssembly.instantiateStreaming(g, e).then(b, function (k) { E("wasm streaming compile failed: " + k); E("falling back to ArrayBuffer instantiation"); return c(b) }) })
          })().catch(ba); return {}
        })(); d.___wasm_call_ctors = function () { return (d.___wasm_call_ctors = d.asm.Q).apply(null, arguments) };
        ...
        function Nc(a) {
          function b() { if (!Kc && (Kc = !0, d.calledRun = !0, !ya)) { z || Za(Ma); z || Za(Na); aa(d); if (d.onRuntimeInitialized) d.onRuntimeInitialized(); if (Oc) { var c = a, e = d._emscripten_proxy_main; c = c || []; var h = c.length + 1, g = Ga(4 * (h + 1)); M[g >> 2] = Fa(ia); for (var k = 1; k < h; k++)M[(g >> 2) + k] = Fa(c[k - 1]); M[(g >> 2) + h] = 0; e(h, g) } if (!z) { if (d.postRun) for ("function" == typeof d.postRun && (d.postRun = [d.postRun]); d.postRun.length;)c = d.postRun.shift(), Oa.unshift(c); Za(Oa) } } } a = a || ha; if (!(0 < Qa)) if (z) aa(d), z || Za(Ma), postMessage({ cmd: "loaded" });
          else { if (!z) { if (d.preRun) for ("function" == typeof d.preRun && (d.preRun = [d.preRun]); d.preRun.length;)Pa(); Za(La) } 0 < Qa || (d.setStatus ? (d.setStatus("Running..."), setTimeout(function () { setTimeout(function () { d.setStatus("") }, 1); b() }, 1)) : b()) }
        } d.run = Nc; function kb(a) { za = a; if (z) throw postMessage({ cmd: "exitProcess", returnCode: a }), new qa(a); ra() || l.Ra(); za = a; if (!ra()) { l.Ra(); if (d.onExit) d.onExit(a); ya = !0 } ja(a, new qa(a)) } if (d.preInit) for ("function" == typeof d.preInit && (d.preInit = [d.preInit]); 0 < d.preInit.length;)d.preInit.pop()();
